...
df = pd.read_csv("../data/Xton_sub_meta.csv")
rna_df = pd.read_csv('../data/Xton_sub_norm.csv')
...
np.save('../data/Xton_train_mask.npy', train_mask)

# bokai 
metaload_path = '../data/'

#df_clean = pd.read_csv(os.path.join(metaload_path,  "features_and_metadata.csv"), index_col=0)
cell_nbhd = np.load(os.path.join(metaload_path,  "Xton_cell_nbhd_res0.5_k20.npy"))
train_mask = np.load(os.path.join(metaload_path, "Xton_train_mask.npy"))
...
#             ind = self.test_list[index]
        ...
        #img = img.transpose(0,3,1,2) # small formatting issue while saving
        if self.use_transform:
...
class CNN_cell512_channel2_layer6(nn.Module):
    def __init__(self):
        ...
        self.fc4 = nn.Linear(128, 8) # change due to cell type labels
...
model_save_path = os.path.join(metaload_path, "Xton_cnn", f"cnn_512_l{l}_layer6", "epochs")
