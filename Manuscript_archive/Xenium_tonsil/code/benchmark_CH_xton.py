...
# quick concact
#features = pd.read_csv('../data/Xton_sub_pc.csv').to_numpy()
#nbhd = np.load('../data/Xton_cell_nbhd_res0.5_k20.npy')
#concat = np.concatenate((features[:,:30], nbhd), axis = 1)#.shape
#np.save('../data/xton_concact.npy', concat)
...
test2 = test.melt(id_vars=['k'], value_vars=['ch_feature', 'ch_snap', 'ch_concact', 'ch_spice', 'ch_muse',
                                             'ch_cca', 'ch_mofa'])
test2.to_csv('../data/CH_result_xton.csv')

plot = sns.lineplot(data=test2,x="k", y="value", hue="variable")
fig = plot.get_figure()
fig.savefig('../plots/CH_allmethods.svg', dpi = 300) 
